...
function isRememberSessionChecked() {
...
function oauthUrl(mode) {
  ...
  if (isRememberSessionChecked()) params.set("remember", "1");
  return `/login?${params}`;
}
...
/** @param {'signin'|'signup'} mode */
function wireRememberCheckbox(mode) {
  const box = document.getElementById("auth-remember");
  if (!(box instanceof HTMLInputElement)) return;
  const stored = localStorage.getItem(REMEMBER_SESSION_KEY);
  if (stored === "1") box.checked = true;
  const persist = () => {
    localStorage.setItem(REMEMBER_SESSION_KEY, box.checked ? "1" : "0");
    const btn = document.getElementById("auth-github-btn");
    if (btn instanceof HTMLAnchorElement) btn.href = oauthUrl(mode);
  };

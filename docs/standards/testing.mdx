...
### Service Testing — Repository Port (In-Memory Adapter)

Use this pattern when a service has been refactored to inject a repository via a port token (e.g., `CONSENT_REPO`). Provide an `InMemoryConsentRepository` as the `useValue` instead of the real Drizzle adapter. Reserve the Drizzle mock pattern above for adapter-level tests — i.e., tests that verify the Drizzle adapter itself.
...
**`{ provide: CONSENT_REPO, useValue: repo }` pattern:** The in-memory repository is instantiated directly (not via DI) so test code has full access to `clear()` and can inspect internal state when needed. NestJS resolves the `CONSENT_REPO` token to the provided instance at compile time.

**`afterEach(() => repo.clear())` for test isolation:** Calling `repo.clear()` before each test ensures every test starts with an empty store. Without this, tests become order-dependent — a record inserted by test A will appear in test B if they share the same repository instance.

**Note:** `InMemoryConsentRepository` must not be decorated with `@Injectable()`. It is registered only via `useValue` or `useClass` in test modules; the decorator would add NestJS metadata that belongs to DI-managed classes only.

**Drizzle adapter tests:** Testing the Drizzle adapter in isolation (verifying that SQL queries are formed correctly) is an optional follow-up. For now, the Drizzle adapter is exercised via integration and E2E tests.

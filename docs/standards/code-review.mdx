...
- [ ] If `@Inject(DRIZZLE)` is used, verify the file is in an allowed path (`*.repository.ts`, `admin/**`, or explicitly listed service) and has a `// RLS-BYPASS: <reason>` inline comment

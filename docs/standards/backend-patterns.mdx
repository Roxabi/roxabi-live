...
```typescript
// consent.repository.ts — port defines both the interface and the DTO
...
The service re-exports the DTO for external consumers: `export type { SaveConsentDto } from './consent.repository.js'`. This ensures the dependency arrow flows inward: adapters → port ← service.

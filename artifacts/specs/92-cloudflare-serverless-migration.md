...
| S5 | Webhook handlers | HMAC verify (null-safe, Web Crypto); **7 events** issues/deps/sub_issues/ref_create/ref_delete/pull_request/**milestone** (rename: `edited`+title-change only, in-place, no re-fetch — postdates orig scope, commit `c5b59cb`); two SQL constants (full vs webhook); **~9 single-row write helpers** (add/removeEdge, upsertEdges per-issue, upsertIssueFromWebhook, replaceLabels, deleteIssue, setActiveBranch, upsertPrState, renameMilestone) — NOT covered by S4 bulk sync; **drop `trigger_heal`** (no in-process reconciler in Workers — webhook D1 writes are authoritative, cron is the reconcile backstop) | `worker/src/webhook/hmac.ts`, `worker/src/webhook/handlers.ts`, `worker/src/webhook/mutations.ts` | 1.5 | S3 | Test delivery from GitHub repo webhooks → staging endpoint; HMAC mismatch → 401/403 (align — Python SSoT uses 401); correct actions update D1 rows; `status` preserved on webhook upsert; milestone rename updates rows in place; unknown event → `{ok:true,ignored:…}` |
